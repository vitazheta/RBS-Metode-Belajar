...
class HasilRekomendasiController extends Controller
{
    ...
    // --- Fungsi Baru: getKategoriText ---
    // Fungsi ini menerjemahkan nilai numerik menjadi label teks
    private function getKategoriText($aspek, $nilai)
    {
        // Pastikan nilai diubah ke float untuk perbandingan akurat
        $nilai = (float) $nilai;

        // Logika kategori sesuai permintaan Anda
        // Perhatikan kembali definisi untuk 'TINGGI' / 'SANGAT X' karena skala 0-3
        // Jika nilai maksimal di database Anda adalah 3, maka 'TINGGI' hanya akan tercapai jika nilai == 3
        if ($aspek === 'akademik') {
            if ($nilai <= 2) return 'Rendah';
            if ($nilai <= 3) return 'Sedang'; // Ini akan mencakup nilai > 2 hingga 3.00
            return 'Tinggi'; // Ini hanya tercapai jika nilai > 3 (misal, 3.1 ke atas), yang tidak mungkin di skala 0-3
        } else if ($aspek === 'sekolah') {
            if ($nilai <= 2) return 'Kurang Mendukung';
            if ($nilai <= 3) return 'Mendukung';
            return 'Sangat Mendukung';
        } else if ($aspek === 'ekonomi') {
            if ($nilai <= 2) return 'Kurang Mencukupi';
            if ($nilai <= 3) return 'Mencukupi';
            return 'Sangat Mencukupi';
        } else if ($aspek === 'perkuliahan') {
            if ($nilai <= 2) return 'Kurang Baik';
            if ($nilai <= 3) return 'Baik';
            return 'Sangat Baik';
        }
        return (string) $nilai; // Kembalikan nilai asli (sebagai string) jika aspek tidak dikenal
    }
    // --- Akhir Fungsi getKategoriText ---
    ...
    private function getAlasanProsesKuliah()
    {
        return [
            ...
            'tinggi-rendah-sedang' => 'SNBP sangat adaptif, SNBT belum aktif, Mandiri cukup mampu menyesuaikan diri.',
            'tinggi-rendah-tinggi' => 'SNBP dan Mandiri sangat aktif dan percaya diri, SNBT masih menghadapi tantangan adaptasi.',
            'tinggi-sedang-rendah' => 'SNBP sangat aktif, SNBT cukup baik, Mandiri belum aktif karena terkendala ekonomi atau',
            ];
    ...
    // --- UBAH DI SINI: Terjemahkan nilai numerik menjadi teks menggunakan nama aspek database ---
        // Kita akan menambahkan atribut baru (misal: akademik_total_text) ke setiap objek mahasiswa
        $mahasiswa->each(function ($mhs) {
            $mhs->akademik_total_text = $this->getKategoriText('akademik', $mhs->akademik_total); // 'akademik'
            $mhs->sekolah_total_text = $this->getKategoriText('sekolah', $mhs->sekolah_total);     // 'sekolah'
            $mhs->ekonomi_total_text = $this->getKategoriText('ekonomi', $mhs->ekonomi_total);     // 'ekonomi'
            $mhs->perkuliahan_total_text = $this->getKategoriText('perkuliahan', $mhs->perkuliahan_total); // 'perkuliahan'
        });
        // --- AKHIR UBAH ---
